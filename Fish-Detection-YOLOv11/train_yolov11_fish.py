...
def check_environment():
    ...
    # 检查数据集
    data_dir = Path(r"C:\Users\cm\Desktop\YOLO11\data")
    config_path = Path(r"C:\Users\cm\Desktop\YOLO11\datasets\custom.yaml")
...
def save_training_summary(training_time, metrics):
    """保存训练摘要"""
    summary_path = Path(r"C:\Users\cm\Desktop\YOLO11\training_summary.txt")
    ...
    ----------
    总图片数: 365
    训练集: {len(list(Path("C:/Users/cm/Desktop/YOLO11/data/images/train").glob('*.*')))} 张
    验证集: {len(list(Path("C:/Users/cm/Desktop/YOLO11/data/images/val").glob('*.*')))} 张
    测试集: {len(list(Path("C:/Users/cm/Desktop/YOLO11/data/images/test").glob('*.*')))} 张
    ...
    模型位置:
    --------
    最佳模型: runs/train/fish_detection_v4/weights/best.pt
    最后模型: runs/train/fish_detection_v4/weights/last.pt
...
def quick_test(model):
    ...
    test_dir = Path(r"C:\Users\cm\Desktop\YOLO11\data\images\test")
